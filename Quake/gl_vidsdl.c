...
static qboolean			emissive_volume_timestamps_written[DOUBLE_BUFFERED];
static qboolean			live_as_timestamps_written[DOUBLE_BUFFERED];

#define TIMESTAMP_QUERY_COUNT				 25
...
#define TIMESTAMP_QUERY_VOLUME_START		 23
#define TIMESTAMP_QUERY_VOLUME_END			 24
...
uint32_t				rs_emissive_volume_gputime_us;
qboolean				rs_emissive_volume_gputime_valid;
...
static void GL_InitCommandBuffers (void)
{
		...
		(vulkan_globals.device_properties.limits.timestampPeriod > 0.0f))
	{
		...
		query_pool_create_info.queryCount = TIMESTAMP_QUERY_COUNT * DOUBLE_BUFFERED;
...
void GL_BeginEmissiveCoarseTimestamp (cb_context_t *cbx)
{
	...
	{
		vkCmdResetQueryPool (
			cbx->cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_EMISSIVE_START, 2);
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_EMISSIVE_START);
	}
}

void GL_ResetEmissiveCoarseTimestamp (void)
{
	memset (emissive_coarse_timestamps_written, 0, sizeof (emissive_coarse_timestamps_written));
	rs_emissive_coarse_gputime_valid = false;
}
...
void GL_EndEmissiveCoarseTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
	{
		vkCmdWriteTimestamp (
			...
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_EMISSIVE_END);
		emissive_coarse_timestamps_written[current_cb_index] = true;
	}
}
...
void GL_BeginEmissiveDetailTimestamp (cb_context_t *cbx)
{
	...
	{
		vkCmdResetQueryPool (
			cbx->cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_DETAIL_START, 2);
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_DETAIL_START);
	}
}

void GL_ResetEmissiveDetailTimestamp (void)
{
	memset (emissive_detail_work_written, 0, sizeof (emissive_detail_work_written));
	rs_emissive_detail_gputime_valid = false;
}
...
void GL_EndEmissiveDetailTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_DETAIL_END);
...
{
	memset (emissive_bounce_refresh_timestamps_written, 0, sizeof (emissive_bounce_refresh_timestamps_written));
	rs_emissive_bounce_refresh_gputime_valid = false;
}
...
void GL_BeginEmissiveBounceRefreshTimestamp (cb_context_t *cbx)
{
	...
	{
		vkCmdResetQueryPool (
			cbx->cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_BOUNCE_REFRESH_START, 2);
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_BOUNCE_REFRESH_START);
	}
}
...
void GL_EndEmissiveBounceRefreshTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_BOUNCE_REFRESH_END);
	emissive_bounce_refresh_timestamps_written[current_cb_index] = true;
}
...
{
	memset (emissive_transient_timestamps_written, 0, sizeof (emissive_transient_timestamps_written));
	memset (emissive_transient_detail_generations, 0, sizeof (emissive_transient_detail_generations));
	rs_emissive_transient_gputime_valid = false;
}
...
void GL_BeginEmissiveVolumeTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
	{
		vkCmdResetQueryPool (cbx->cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_VOLUME_START, 2);
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_VOLUME_START);
	}
}

void GL_ResetEmissiveVolumeTimestamp (void)
{
	memset (emissive_volume_timestamps_written, 0, sizeof (emissive_volume_timestamps_written));
	rs_emissive_volume_gputime_valid = false;
}

void GL_EndEmissiveVolumeTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
	{
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_VOLUME_END);
		emissive_volume_timestamps_written[current_cb_index] = true;
	}
}
...
void GL_BeginLiveASTimestamp (cb_context_t *cbx)
{
	...
	{
		vkCmdResetQueryPool (
			cbx->cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_LIVE_AS_START, 2);
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_LIVE_AS_START);
	}
}

void GL_ResetLiveASTimestamp (void)
{
	memset (live_as_timestamps_written, 0, sizeof (live_as_timestamps_written));
	rs_live_as_gputime_us = 0;
	rs_live_as_gputime_valid = false;
}
...
void GL_EndLiveASTimestamp (cb_context_t *cbx)
{
	if (timestamp_query_pool != VK_NULL_HANDLE)
	{
		vkCmdWriteTimestamp (
			cbx->cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_LIVE_AS_END);
		live_as_timestamps_written[current_cb_index] = true;
	}
}
...
void GL_BeginRenderingTask (void *unused)
{
	...
	// the fence wait above guarantees the timestamps and detail work from the submission that used this slot are complete
	...
	if ((timestamp_query_pool != VK_NULL_HANDLE) && emissive_coarse_timestamps_written[current_cb_index])
	{
		uint64_t timestamps[2];
		if (vkGetQueryPoolResults (
				vulkan_globals.device, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_EMISSIVE_START, 2, sizeof (timestamps),
	...
	if (emissive_detail_work_written[current_cb_index])
	{
		if (timestamp_query_pool != VK_NULL_HANDLE)
		{
			uint64_t timestamps[2];
			if (vkGetQueryPoolResults (
					vulkan_globals.device, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_DETAIL_START, 2,
	...
	if (emissive_bounce_refresh_timestamps_written[current_cb_index])
	{
		if (timestamp_query_pool != VK_NULL_HANDLE)
		{
			uint64_t timestamps[2];
	...
	if (emissive_transient_timestamps_written[current_cb_index])
	{
		if (timestamp_query_pool != VK_NULL_HANDLE)
		{
			uint64_t timestamps[2];
			if (vkGetQueryPoolResults (
					vulkan_globals.device, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_TRANSIENT_START, 2,
					sizeof (timestamps), timestamps, sizeof (uint64_t), VK_QUERY_RESULT_64_BIT) == VK_SUCCESS)
			{
		...
		if (emissive_transient_detail_generations[current_cb_index])
		{
	...
	if (emissive_volume_timestamps_written[current_cb_index])
	{
		if (timestamp_query_pool != VK_NULL_HANDLE)
		{
			uint64_t timestamps[2];
			if (vkGetQueryPoolResults (
					vulkan_globals.device, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_VOLUME_START, 2,
					sizeof (timestamps), timestamps, sizeof (uint64_t), VK_QUERY_RESULT_64_BIT) == VK_SUCCESS)
			{
				rs_emissive_volume_gputime_us =
					(uint32_t)((double)(timestamps[1] - timestamps[0]) * (double)vulkan_globals.device_properties.limits.timestampPeriod / 1000.0);
				rs_emissive_volume_gputime_valid = true;
				RTPerf_Record ("gpu_volume", (double)rs_emissive_volume_gputime_us / 1000.0, 0);
			}
		}
		emissive_volume_timestamps_written[current_cb_index] = false;
	}
	...
	if ((timestamp_query_pool != VK_NULL_HANDLE) && live_as_timestamps_written[current_cb_index])
	{
		uint64_t timestamps[2];
		if (vkGetQueryPoolResults (
				vulkan_globals.device, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_LIVE_AS_START, 2,
				sizeof (timestamps), timestamps, sizeof (uint64_t), VK_QUERY_RESULT_64_BIT) == VK_SUCCESS)
		{
	...
	if (timestamp_query_pool != VK_NULL_HANDLE)
	{
		...
		vkCmdResetQueryPool (
			first_cb, timestamp_query_pool, (current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_FRAME_START, 2);
		vkCmdWriteTimestamp (
			first_cb, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestamp_query_pool,
			(current_cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_FRAME_START);
	}
...
static void GL_EndRenderingTask (end_rendering_parms_t *parms)
{
	...
	{
		vkCmdWriteTimestamp (
			render_passes_cb, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestamp_query_pool, (cb_index * TIMESTAMP_QUERY_COUNT) + TIMESTAMP_QUERY_FRAME_END);
