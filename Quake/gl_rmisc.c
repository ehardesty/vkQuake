...
// 27 established floats plus the 5-float RT-emissive volume sampling rect
// (viewport x/y/w/h, z extent) appended by world.frag. 32 floats is exactly
// the guaranteed 128-byte push-constant limit.
#define MAIN_GRAPHICS_PUSH_CONSTANT_FLOATS 32
...
void R_CreateDescriptorSetLayouts ()
{
...

	{
		...
		emissive_coarse_layout_bindings[8].binding = num_descriptors++;
...

	{
		...
		for (int i = 1; i < 5; ++i)
		{
			...
			emissive_volume_layout_bindings[i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
			emissive_volume_layout_bindings[i].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
		}

		descriptor_set_layout_create_info.bindingCount = num_descriptors;
		descriptor_set_layout_create_info.pBindings = emissive_volume_layout_bindings;
		...
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreateDescriptorSetLayout failed with code %i", (int)err);
		GL_SetObjectName ((uint64_t)vulkan_globals.emissive_volume_set_layout.handle, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, "emissive volume");
	}
	if (vulkan_globals.ray_query)
	{
...
void R_CreateDescriptorPool ()
{
	...
	pool_sizes[4].descriptorCount = MIN_NB_DESCRIPTORS_PER_TYPE + MAX_SANITY_LIGHTMAPS * 25;
	...
	pool_sizes[7].descriptorCount = MIN_NB_DESCRIPTORS_PER_TYPE + (7 + MAXLIGHTMAPS * 3 / 4) * MAX_SANITY_LIGHTMAPS;
...
void R_CreatePipelineLayouts ()
{
	...
	if (vulkan_globals.ray_query)
	{
		...
		VkDescriptorSetLayout volume_descriptor_set_layouts[1] = {vulkan_globals.emissive_volume_set_layout.handle};
		ZEROED_STRUCT (VkPushConstantRange, volume_push_constant_range);
		volume_push_constant_range.size = sizeof (emissive_volume_push_t);
		volume_push_constant_range.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
		ZEROED_STRUCT (VkPipelineLayoutCreateInfo, volume_layout_create_info);
		volume_layout_create_info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
		volume_layout_create_info.setLayoutCount = countof (volume_descriptor_set_layouts);
		volume_layout_create_info.pSetLayouts = volume_descriptor_set_layouts;
		volume_layout_create_info.pushConstantRangeCount = 1;
		volume_layout_create_info.pPushConstantRanges = &volume_push_constant_range;
		err = vkCreatePipelineLayout (
			vulkan_globals.device, &volume_layout_create_info, NULL, &vulkan_globals.emissive_volume_pipeline.layout.handle);
		if (err != VK_SUCCESS)
			Sys_Error ("vkCreatePipelineLayout failed with code %i", (int)err);
		GL_SetObjectName (
...
static VkVertexInputAttributeDescription basic_emissive_vertex_input_attribute_descriptions[4];
static VkVertexInputBindingDescription	 basic_emissive_vertex_binding_descriptions[2];
...
DECLARE_SHADER_MODULE (basic_emissive_vertex_vert);
DECLARE_SHADER_MODULE (basic_emissive_vertex_frag);
DECLARE_SHADER_MODULE (basic_emissive_vertex_oit_frag);
DECLARE_SHADER_MODULE (basic_emissive_vertex_mboit_moment_frag);
DECLARE_SHADER_MODULE (basic_emissive_vertex_mboit_composite_frag);
DECLARE_SHADER_MODULE (basic_emissive_vertex_mboit_composite_msaa_frag);
...
{
	{
		...
		memcpy (
			basic_emissive_vertex_input_attribute_descriptions, basic_vertex_input_attribute_descriptions,
			sizeof (basic_vertex_input_attribute_descriptions));
		basic_emissive_vertex_input_attribute_descriptions[3].binding = 1;
		basic_emissive_vertex_input_attribute_descriptions[3].format = VK_FORMAT_R32G32B32_SFLOAT;
		basic_emissive_vertex_input_attribute_descriptions[3].location = 3;
		basic_emissive_vertex_input_attribute_descriptions[3].offset = 0;
		basic_emissive_vertex_binding_descriptions[0] = basic_vertex_binding_description;
		basic_emissive_vertex_binding_descriptions[1].binding = 1;
		basic_emissive_vertex_binding_descriptions[1].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
		basic_emissive_vertex_binding_descriptions[1].stride = sizeof (vec3_t);
	}
...
static void R_CreateParticlesPipelines ()
{
	...
	infos.shader_stages[0].module = basic_emissive_vertex_vert_module;
	infos.shader_stages[1].module = basic_emissive_vertex_frag_module;
	infos.vertex_input_state.vertexAttributeDescriptionCount = countof (basic_emissive_vertex_input_attribute_descriptions);
	infos.vertex_input_state.pVertexAttributeDescriptions = basic_emissive_vertex_input_attribute_descriptions;
	infos.vertex_input_state.vertexBindingDescriptionCount = countof (basic_emissive_vertex_binding_descriptions);
	infos.vertex_input_state.pVertexBindingDescriptions = basic_emissive_vertex_binding_descriptions;
	R_CreateGraphicsPipeline (&vulkan_globals.particle_emissive_pipeline, &infos, vulkan_globals.basic_pipeline_layout, "particles_emissive");
	...
	infos.shader_stages[0].module = basic_emissive_vertex_vert_module;
	infos.shader_stages[1].module = basic_emissive_vertex_oit_frag_module;
	infos.vertex_input_state.vertexAttributeDescriptionCount = countof (basic_emissive_vertex_input_attribute_descriptions);
	infos.vertex_input_state.pVertexAttributeDescriptions = basic_emissive_vertex_input_attribute_descriptions;
	infos.vertex_input_state.vertexBindingDescriptionCount = countof (basic_emissive_vertex_binding_descriptions);
	infos.vertex_input_state.pVertexBindingDescriptions = basic_emissive_vertex_binding_descriptions;
	R_CreateGraphicsPipeline (&vulkan_globals.particle_emissive_oit_pipeline, &infos, vulkan_globals.basic_pipeline_layout, "particles_emissive_oit");
	...
	infos.shader_stages[0].module = basic_emissive_vertex_vert_module;
	infos.shader_stages[1].module = basic_emissive_vertex_mboit_moment_frag_module;
	infos.vertex_input_state.vertexAttributeDescriptionCount = countof (basic_emissive_vertex_input_attribute_descriptions);
	infos.vertex_input_state.pVertexAttributeDescriptions = basic_emissive_vertex_input_attribute_descriptions;
	infos.vertex_input_state.vertexBindingDescriptionCount = countof (basic_emissive_vertex_binding_descriptions);
	infos.vertex_input_state.pVertexBindingDescriptions = basic_emissive_vertex_binding_descriptions;
	R_CreateGraphicsPipeline (
		&vulkan_globals.particle_emissive_mboit_moment_pipeline, &infos, vulkan_globals.basic_pipeline_layout, "particles_emissive_mboit_moment");
	...
	infos.shader_stages[1].module = (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT)
		? basic_emissive_vertex_mboit_composite_frag_module
		: basic_emissive_vertex_mboit_composite_msaa_frag_module;
	infos.vertex_input_state.vertexAttributeDescriptionCount = countof (basic_emissive_vertex_input_attribute_descriptions);
	infos.vertex_input_state.pVertexAttributeDescriptions = basic_emissive_vertex_input_attribute_descriptions;
	infos.vertex_input_state.vertexBindingDescriptionCount = countof (basic_emissive_vertex_binding_descriptions);
	infos.vertex_input_state.pVertexBindingDescriptions = basic_emissive_vertex_binding_descriptions;
	R_CreateGraphicsPipeline (
		&vulkan_globals.particle_emissive_mboit_composite_pipeline, &infos, vulkan_globals.basic_pipeline_layout, "particles_emissive_mboit_composite");
}
...
static void R_CreateWorldPipelines ()
{
	...
	VkSpecializationMapEntry specialization_entries[8];
	...
	specialization_entries[6].offset = 24;
	specialization_entries[6].size = 4;
	specialization_entries[7].constantID = 7;
	specialization_entries[7].offset = 28;
	specialization_entries[7].size = 4;
	uint32_t specialization_data[8];
	...
	specialization_data[6] = 0;
	specialization_data[7] = 0;

	VkSpecializationInfo specialization_info;
	specialization_info.mapEntryCount = countof (specialization_entries) - 3;
	...
	pipeline_create_infos_t infos;
	for (int emissive_detail = 0; emissive_detail < 2; ++emissive_detail)
	{
		for (int emissive_bandlimit = 0; emissive_bandlimit < 2; ++emissive_bandlimit)
		{
			if (emissive_bandlimit && !emissive_detail)
				continue;
			for (int emissive_coarse = 0; emissive_coarse < 2; ++emissive_coarse)
			{
				if (emissive_detail && !emissive_coarse)
					continue;
						...
						for (int fullbright_enabled = 0; fullbright_enabled < 2; ++fullbright_enabled)
						{
							for (int quantize_lm = 0; quantize_lm < 2; ++quantize_lm)
							{
								const int pipeline_index = fullbright_enabled + (alpha_test * 2) + (alpha_blend * 4) + (quantize_lm * 8) +
														   (emissive_coarse * 16) + (emissive_detail * 32) + (emissive_bandlimit * 64);
								...
								specialization_data[6] = emissive_detail;
								specialization_data[7] = emissive_bandlimit;
								specialization_info.mapEntryCount = emissive_coarse ? countof (specialization_entries) : countof (specialization_entries) - 3;
								for (int variant = 0; variant < MAIN_RENDER_PASS_VARIANT_COUNT; ++variant)
								{
									R_CopyPipelineCreateInfos (&infos, &base);
									infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[variant][MAIN_RENDER_PASS_STENCIL_CLEAR];
									infos.shader_stages[1].module = emissive_coarse
										? (emissive_bandlimit ? world_emissive_bandlimit_frag_module : world_emissive_frag_module)
										: world_frag_module;
								...
								if (alpha_blend)
								{
									...
									R_CopyPipelineCreateInfos (&infos, &base);
									infos.graphics_pipeline.renderPass =
										vulkan_globals.main_render_pass[MAIN_RENDER_PASS_MBOIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
									infos.graphics_pipeline.subpass = 1;
									infos.color_blend_state.attachmentCount = MBOIT_MOMENT_COLOR_ATTACHMENT_COUNT;
									...
									infos.color_blend_state.attachmentCount = MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;
									infos.shader_stages[1].module = (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT)
																		? world_mboit_composite_frag_module
...
static void R_CreateWorldOITVolumePipelines (void)
{
	pipeline_create_infos_t base;
	pipeline_create_infos_t infos;
	...
	volume_spec_info.mapEntryCount = 0;
	...
	// OIT color passes never combine with emissive-atlas bits (blended draws
	// carry no surface atlas), mirroring the base creation skips.
	for (pipeline_index = 0; pipeline_index < WORLD_PIPELINE_COUNT; ++pipeline_index)
	{
		if (!(pipeline_index & 4) || (pipeline_index & (16 | 32 | 64)))
			continue;
		volume_spec_data[0] = (uint32_t)(pipeline_index & 1);
		volume_spec_data[1] = (uint32_t)((pipeline_index & 2) != 0);
		volume_spec_data[2] = 1;
		volume_spec_data[3] = (uint32_t)((pipeline_index & 8) != 0);
		...
		for (scatter_only = 0; scatter_only < 2; ++scatter_only)
		{
			...
			infos.graphics_pipeline.subpass = 1;
			infos.color_blend_state.attachmentCount = WBOIT_COLOR_ATTACHMENT_COUNT;
			infos.shader_stages[1].module = world_oit_volume_frag_module;
			infos.shader_stages[1].pSpecializationInfo = &volume_spec_info;
			infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
			R_SetWBOITBlend (infos.blend_attachment_states);
			R_CreateGraphicsPipeline (
				&vulkan_globals.world_oit_volume_pipelines[pipeline_index][scatter_only], &infos, vulkan_globals.world_pipeline_layout,
				va ("world_oit_volume %d%s", pipeline_index, scatter_only ? " scatter" : ""));
			R_CopyPipelineCreateInfos (&infos, &base);
			infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_MBOIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
			infos.graphics_pipeline.subpass = 2;
			infos.color_blend_state.attachmentCount = MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;
			infos.shader_stages[1].module = (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT) ? world_mboit_composite_volume_frag_module
																																													: world_mboit_composite_volume_msaa_frag_module;
				infos.shader_stages[1].pSpecializationInfo = &volume_spec_info;
				infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
				R_SetMBOITCompositeBlend (infos.blend_attachment_states);
				R_CreateGraphicsPipeline (
					&vulkan_globals.world_mboit_composite_volume_pipelines[pipeline_index][scatter_only], &infos, vulkan_globals.world_pipeline_layout,
					va ("world_mboit_composite_volume %d%s", pipeline_index, scatter_only ? " scatter" : ""));
		}
	}
}
...
static void R_CreateWorldOITVolumePipelines (void);
static void R_CreateLiquidOITVolumePipelines (void);
static void R_CreateAliasOITVolumePipelines (void);
static void R_CreateMD5OITVolumePipelineSet (
	vulkan_pipeline_t oit_pipelines[MODEL_PIPELINE_COUNT][2], vulkan_pipeline_t composite_pipelines[MODEL_PIPELINE_COUNT][2],
	VkVertexInputAttributeDescription *vertex_attributes, uint32_t vertex_attribute_count, VkVertexInputBindingDescription *vertex_binding,
	VkShaderModule vertex_module, const char *name);
...
void R_CreateEmissiveVolumePipelines (void)
{
	pipeline_create_infos_t base;
	pipeline_create_infos_t infos;
	VkSpecializationMapEntry volume_spec_entries[9];
	uint32_t volume_spec_data[9];
	VkSpecializationInfo volume_spec_info;
	int emissive_coarse;
	int emissive_detail;
	...
	for (i = 6; i < 9; ++i)
	{
		volume_spec_entries[i].constantID = (uint32_t)(i - 1);
		volume_spec_entries[i].offset = (uint32_t)((i - 1) * 4);
		volume_spec_entries[i].size = 4;
	}
	volume_spec_info.mapEntryCount = 0;
	volume_spec_info.pMapEntries = volume_spec_entries;
	...
	memset (volume_spec_data, 0, sizeof (volume_spec_data));
	volume_spec_data[4] = vulkan_globals.color_format == VK_FORMAT_A2B10G10R10_UNORM_PACK32;
	for (emissive_coarse = 0; emissive_coarse < 2; ++emissive_coarse)
		for (emissive_detail = 0; emissive_detail < 2; ++emissive_detail)
		{
			if (emissive_detail && !emissive_coarse)
				continue;
			for (alpha_test = 0; alpha_test < 2; ++alpha_test)
				for (fullbright_enabled = 0; fullbright_enabled < 2; ++fullbright_enabled)
					for (quantize_lm = 0; quantize_lm < 2; ++quantize_lm)
						...
						volume_spec_data[7] = 0;
						volume_spec_info.mapEntryCount = emissive_coarse ? 9 : 6;
	...
	R_CreateWorldOITVolumePipelines ();
	R_CreateLiquidOITVolumePipelines ();
	R_CreateAliasOITVolumePipelines ();
	R_CreateMD5OITVolumePipelineSet (
		vulkan_globals.md5_oit_volume_pipelines, vulkan_globals.md5_mboit_composite_volume_pipelines,
		md5_vertex_input_attribute_descriptions, countof (md5_vertex_input_attribute_descriptions), &md5_vertex_binding_description, md5_vert_module,
		"md5");
	R_CreateMD5OITVolumePipelineSet (
		vulkan_globals.md5_8_oit_volume_pipelines, vulkan_globals.md5_8_mboit_composite_volume_pipelines,
		md5_8_vertex_input_attribute_descriptions, countof (md5_8_vertex_input_attribute_descriptions), &md5_8_vertex_binding_description,
		md5_8_vert_module, "md5_8");
	...
	for (alpha_blend = 0; alpha_blend < 2; ++alpha_blend)
		for (quantize_lm = 0; quantize_lm < 2; ++quantize_lm)
		{
			...
			for (scatter_only = 0; scatter_only < 2; ++scatter_only)
			{
				...
				infos.graphics_pipeline.subpass = 1;
				infos.color_blend_state.attachmentCount = WBOIT_COLOR_ATTACHMENT_COUNT;
				infos.shader_stages[1].module = world_liquid_volume_oit_frag_module;
				infos.shader_stages[1].pSpecializationInfo = &volume_spec_info;
				infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
				R_SetWBOITBlend (infos.blend_attachment_states);
				R_CreateGraphicsPipeline (
					&vulkan_globals.liquid_oit_volume_pipelines[liquid_pipeline_index][scatter_only], &infos,
					vulkan_globals.world_pipeline_layout, va ("liquid_oit_volume %d%s", liquid_pipeline_index, scatter_only ? " scatter" : ""));
				R_CopyPipelineCreateInfos (&infos, &base);
				infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_MBOIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
				infos.graphics_pipeline.subpass = 2;
				infos.color_blend_state.attachmentCount = MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;
				infos.shader_stages[1].module = (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT)
					? world_liquid_volume_mboit_composite_frag_module
					: world_liquid_volume_mboit_composite_msaa_frag_module;
				infos.shader_stages[1].pSpecializationInfo = &volume_spec_info;
				infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
				R_SetMBOITCompositeBlend (infos.blend_attachment_states);
				R_CreateGraphicsPipeline (
					&vulkan_globals.liquid_mboit_composite_volume_pipelines[liquid_pipeline_index][scatter_only], &infos,
					vulkan_globals.world_pipeline_layout, va ("liquid_mboit_composite_volume %d%s", liquid_pipeline_index, scatter_only ? " scatter" : ""));
			}
		}
}
...
static void R_CreateAliasOITVolumePipelines (void)
{
	...
	for (pipeline_index = 0; pipeline_index < MODEL_PIPELINE_SHOWTRIS; ++pipeline_index)
	{
		...
		for (scatter_only = 0; scatter_only < 2; ++scatter_only)
		{
			...
			infos.graphics_pipeline.subpass = 1;
			infos.color_blend_state.attachmentCount = WBOIT_COLOR_ATTACHMENT_COUNT;
			infos.shader_stages[1].module = alpha_test ? alias_alphatest_oit_volume_frag_module : alias_oit_volume_frag_module;
			infos.shader_stages[1].pSpecializationInfo = &spec_info;
			infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
			R_SetWBOITBlend (infos.blend_attachment_states);
			R_CreateGraphicsPipeline (
				&vulkan_globals.alias_oit_volume_pipelines[pipeline_index][scatter_only], &infos, vulkan_globals.alias_volume_pipeline_layout,
				va ("alias_oit_volume %d%s", pipeline_index, scatter_only ? " scatter" : ""));
			R_CopyPipelineCreateInfos (&infos, &base);
			infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_MBOIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
			infos.graphics_pipeline.subpass = 2;
			infos.color_blend_state.attachmentCount = MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;
			if (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT)
				infos.shader_stages[1].module = alpha_test ? alias_alphatest_mboit_composite_volume_frag_module : alias_mboit_composite_volume_frag_module;
			else
				infos.shader_stages[1].module = alpha_test ? alias_alphatest_mboit_composite_volume_msaa_frag_module : alias_mboit_composite_volume_msaa_frag_module;
			infos.shader_stages[1].pSpecializationInfo = &spec_info;
			infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
			R_SetMBOITCompositeBlend (infos.blend_attachment_states);
			R_CreateGraphicsPipeline (
				&vulkan_globals.alias_mboit_composite_volume_pipelines[pipeline_index][scatter_only], &infos,
				vulkan_globals.alias_volume_pipeline_layout, va ("alias_mboit_composite_volume %d%s", pipeline_index, scatter_only ? " scatter" : ""));
		}
	}
}
	...
	VkShaderModule vertex_module, const char *name)
{
	...
	for (pipeline_index = 0; pipeline_index < MODEL_PIPELINE_SHOWTRIS; ++pipeline_index)
	{
		...
		for (scatter_only = 0; scatter_only < 2; ++scatter_only)
		{
			...
			infos.graphics_pipeline.subpass = 1;
			infos.color_blend_state.attachmentCount = WBOIT_COLOR_ATTACHMENT_COUNT;
			infos.shader_stages[1].module = alpha_test ? md5_alphatest_oit_volume_frag_module : md5_oit_volume_frag_module;
			infos.shader_stages[1].pSpecializationInfo = &spec_info;
			infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
			R_SetWBOITBlend (infos.blend_attachment_states);
			R_CreateGraphicsPipeline (&oit_pipelines[pipeline_index][scatter_only], &infos, vulkan_globals.md5_volume_pipeline_layout,
				va ("%s_oit_volume %d%s", name, pipeline_index, scatter_only ? " scatter" : ""));
			R_CopyPipelineCreateInfos (&infos, &base);
			infos.graphics_pipeline.renderPass = vulkan_globals.main_render_pass[MAIN_RENDER_PASS_MBOIT][MAIN_RENDER_PASS_STENCIL_CLEAR];
			infos.graphics_pipeline.subpass = 2;
			infos.color_blend_state.attachmentCount = MBOIT_COMPOSITE_COLOR_ATTACHMENT_COUNT;
			if (vulkan_globals.sample_count == VK_SAMPLE_COUNT_1_BIT)
				infos.shader_stages[1].module = alpha_test ? md5_alphatest_mboit_composite_volume_frag_module : md5_mboit_composite_volume_frag_module;
			else
				infos.shader_stages[1].module = alpha_test ? md5_alphatest_mboit_composite_volume_msaa_frag_module : md5_mboit_composite_volume_msaa_frag_module;
			infos.shader_stages[1].pSpecializationInfo = &spec_info;
			infos.depth_stencil_state.depthWriteEnable = VK_FALSE;
			R_SetMBOITCompositeBlend (infos.blend_attachment_states);
			R_CreateGraphicsPipeline (&composite_pipelines[pipeline_index][scatter_only], &infos, vulkan_globals.md5_volume_pipeline_layout,
				va ("%s_mboit_composite_volume %d%s", name, pipeline_index, scatter_only ? " scatter" : ""));
		}
	}
}
...
void R_DestroyEmissiveVolumePipelines (void)
{
	...
	for (i = 0; i < WORLD_PIPELINE_COUNT; ++i)
		for (s = 0; s < 2; ++s)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.world_oit_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.world_oit_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.world_mboit_composite_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.world_mboit_composite_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
		}
	for (i = 0; i < LIQUID_EMISSIVE_PIPELINE_COUNT; ++i)
		for (s = 0; s < 2; ++s)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.liquid_oit_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.liquid_oit_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.liquid_mboit_composite_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.liquid_mboit_composite_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
		}
	for (i = 0; i < MODEL_PIPELINE_COUNT; ++i)
		for (s = 0; s < 2; ++s)
		{
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.alias_oit_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.alias_oit_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.alias_mboit_composite_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.alias_mboit_composite_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_oit_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.md5_oit_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_mboit_composite_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.md5_mboit_composite_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_8_oit_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.md5_8_oit_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
			vkDestroyPipeline (vulkan_globals.device, vulkan_globals.md5_8_mboit_composite_volume_pipelines[i][s].handle, NULL);
			vulkan_globals.md5_8_mboit_composite_volume_pipelines[i][s].handle = VK_NULL_HANDLE;
		}
	for (i = 0; i < WORLD_PIPELINE_COUNT; ++i)
		for (variant = 0; variant < MAIN_RENDER_PASS_VARIANT_COUNT; ++variant)
			for (s = 0; s < 2; ++s)
...
static void R_CreateUpdateLightmapPipelines ()
{
	...
	if (vulkan_globals.ray_query)
	{
		...
		const VkSpecializationMapEntry bandlimit_radiance_overlay_entries[] = {
			{1, 0, sizeof (uint32_t)},
			{2, sizeof (uint32_t), sizeof (uint32_t)},
			{4, 2 * sizeof (uint32_t), sizeof (uint32_t)},
			{5, 3 * sizeof (uint32_t), sizeof (uint32_t)},
		};
		const uint32_t bandlimit_radiance_overlay_data[] = {true, true, true, true};
		const VkSpecializationInfo bandlimit_radiance_overlay_specialization = {
			countof (bandlimit_radiance_overlay_entries), bandlimit_radiance_overlay_entries, sizeof (bandlimit_radiance_overlay_data),
			bandlimit_radiance_overlay_data};
		vulkan_globals.emissive_bandlimit_radiance_overlay_pipeline.layout = vulkan_globals.emissive_coarse_pipeline.layout;
		R_CreateComputePipeline (
			&vulkan_globals.emissive_bandlimit_radiance_overlay_pipeline, emissive_coarse_comp_module, 0,
			&bandlimit_radiance_overlay_specialization, "emissive_bandlimit_radiance_overlay");
...
static void R_CreateShaderModules ()
{
	...
	CREATE_SHADER_MODULE (basic_emissive_vertex_vert);
	CREATE_SHADER_MODULE (basic_emissive_vertex_frag);
	CREATE_SHADER_MODULE (basic_emissive_vertex_oit_frag);
	CREATE_SHADER_MODULE (basic_emissive_vertex_mboit_moment_frag);
	CREATE_SHADER_MODULE (basic_emissive_vertex_mboit_composite_frag);
	CREATE_SHADER_MODULE_COND (basic_emissive_vertex_mboit_composite_msaa_frag, vulkan_globals.sample_count != VK_SAMPLE_COUNT_1_BIT);
...
static void R_DestroyShaderModules ()
{
	...
	DESTROY_SHADER_MODULE (basic_emissive_vertex_vert);
	DESTROY_SHADER_MODULE (basic_emissive_vertex_frag);
	DESTROY_SHADER_MODULE (basic_emissive_vertex_oit_frag);
	DESTROY_SHADER_MODULE (basic_emissive_vertex_mboit_moment_frag);
	DESTROY_SHADER_MODULE (basic_emissive_vertex_mboit_composite_frag);
	DESTROY_SHADER_MODULE (basic_emissive_vertex_mboit_composite_msaa_frag);
...
void R_DestroyPipelines (void)
{
	...
	R_DestroyEmissiveVolumePipelines ();
	...
	vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_emissive_pipeline.handle, NULL);
	vulkan_globals.particle_emissive_pipeline.handle = VK_NULL_HANDLE;
	...
	if (vulkan_globals.particle_emissive_oit_pipeline.handle != VK_NULL_HANDLE)
	{
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_emissive_oit_pipeline.handle, NULL);
		vulkan_globals.particle_emissive_oit_pipeline.handle = VK_NULL_HANDLE;
	}
	for (int variant = 0; variant < MAIN_RENDER_PASS_VARIANT_COUNT; ++variant)
	{
	...
	vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_emissive_mboit_moment_pipeline.handle, NULL);
	vulkan_globals.particle_emissive_mboit_moment_pipeline.handle = VK_NULL_HANDLE;
	...
	vkDestroyPipeline (vulkan_globals.device, vulkan_globals.particle_emissive_mboit_composite_pipeline.handle, NULL);
	vulkan_globals.particle_emissive_mboit_composite_pipeline.handle = VK_NULL_HANDLE;
#ifdef PSET_SCRIPT
	for (i = 0; i < 8; ++i)
	{
		for (int variant = 0; variant < MAIN_RENDER_PASS_VARIANT_COUNT; ++variant)
	...
	if (vulkan_globals.emissive_bandlimit_detail_pipeline.handle != VK_NULL_HANDLE)
	{
		...
		vkDestroyPipeline (vulkan_globals.device, vulkan_globals.emissive_bandlimit_radiance_overlay_pipeline.handle, NULL);
		vulkan_globals.emissive_bandlimit_radiance_overlay_pipeline.handle = VK_NULL_HANDLE;
	...
	if (vulkan_globals.emissive_transient_detail_pipeline.handle != VK_NULL_HANDLE)
	{
		...
		vulkan_globals.emissive_transient_detail_pipeline.handle = VK_NULL_HANDLE;
	}
...
VkDescriptorSet R_AllocateDescriptorSet (vulkan_desc_set_layout_t *layout)
{
	...
	VkResult		err = vkAllocateDescriptorSets (vulkan_globals.device, &descriptor_set_allocate_info, &handle);
	if (err != VK_SUCCESS)
		Sys_Error ("vkAllocateDescriptorSets failed with code %i", (int)err);
